...
function checkAndCreateProfile(accountId) {
	if (
		!fs.existsSync(
			`${backendConfig.directory}/acc/profiles/${accountId}.json`
		)
	) {
		fs.copyFileSync(
			`${backendConfig.directory}/acc/templates/config.json`,
			`${backendConfig.directory}/acc/profiles/${accountId}.json`
		);
	}
}
...
function loadProfile(profileId) {
	return JSON.parse(
		fs.readFileSync(
			`${backendConfig.directory}/acc/templates/profile_${profileId}.json`
		)
	);
}
...
function saveConfig(directory, accountId, data) {
	fs.writeFileSync(
		`${directory}/acc/profiles/${accountId}.json`,
